...
def draw():
    ...
    for i in range(len(fruit_inventory)):
        ...
        sum_val += fruit_inventory[i]  # add the value of the inventory each time through the loop

    fill(240)
    average = sum_val/len(fruit_inventory)
    text("Average Number of Fruits: " + str(int(average)), width/2, height/2 + 100)
